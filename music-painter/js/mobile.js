// Mobile mini keyboard + drum pad (no HTML changes required)
(function(MP){
  // quick mobile detect (needs MP.isMobile)
  if (!MP.isMobile) return;

  // inject minimal styles
  const css = `
  .mp-pad__toggle{position:fixed;right:10px;bottom:10px;z-index:10000;background:#17233a;border:1px solid #2a3b52;border-radius:14px;padding:8px 10px;font-size:13px;color:#e8f0ff}
  .mp-pad{position:fixed;left:0;right:0;bottom:0;z-index:10001;background:#0e1622;border-top:1px solid #223049;box-shadow:0 -10px 30px rgba(0,0,0,.35);transform:translateY(100%);transition:transform .22s ease}
  ...
  .mp-pad__btn{user-select:none;-webkit-user-select:none;touch-action:none;padding:14px 8px;border:1px solid #263142;border-radius:12px;background:#0f172a;text-align:center;color:#e8f0ff}
  ...
  const pad=document.createElement('div'); pad.className='mp-pad'; pad.innerHTML=`
  ...
  const PADS =[40,41,42,43,36,37,38,39];                // your mapping
  ...
  function build(){
    ...
    if (layout==='piano'){
      ...
      PADS.forEach(n=>{
        const t=MP.PAD_TO_TYPE[n]; const btn=document.createElement('button'); btn.className='mp-pad__btn';
