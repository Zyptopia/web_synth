...
function updateInlineBadges(){ for(const p of MapState.ccParams()){ const el=$('#'+p); if(!el) continue; const row=el.closest('.control'); if(!row) continue; const b=row.querySelector('.miniMap'); if(!b) continue; const cc=MapState.ccMap[p]; const mode=(cc!=null && MapState.ccMode[cc])?MapState.ccMode[cc]:null; b.textContent=cc!=null?`●CC${cc}`:'●'; b.title=cc!=null?`Mapped to CC${cc}${mode?` (${mode})`:''}. Click to re-map or press Esc to cancel.`:'Map this control (Learn)'; }
...
function hookSlider(param,fmt,on){ const el=$('#'+param), lab=$('#'+param+'Val'); const apply=()=>{ const v=+el.value; lab.textContent=fmt(v); const r=PARAM_RANGES[param]; const x=(v-r.min)/(r.max-r.min); MapState.setParamNorm(param,x); on(v); updateInlineBadges(); }; el.addEventListener('input',apply); apply() }
...
function renderCCTable(){ const params=MapState.ccParams(); const table=$('#ccTable'); if(!table) return; table.innerHTML=`<tr><th>Parameter</th><th>CC#</th><th>Mode</th><th>Learn</th></tr>`+params.map(p=>{ const cc=MapState.ccMap[p]??''; const mode=cc!=='' && MapState.ccMode[cc]?MapState.ccMode[cc]:''; return `<tr><td>${p}</td><td><input data-cc-param="${p}" class="ccNum" type="number" min="0" max="127" value="${cc}"></td><td>${mode}</td><td><button class="learnCC" data-param="${p}">●</button></td></tr>` }).join(''); table.querySelectorAll('.ccNum').forEach(inp=>{ inp.onchange=()=>{ MapState.setCC(inp.dataset.ccParam, clamp(parseInt(inp.value,10)||0,0,127)); saveAll(); updateInlineBadges(); }; }); table.querySelectorAll('.learnCC').forEach(btn=>{ btn.onclick=()=>{ startCCLearn(btn.dataset.param, btn); }; }); }
function renderPadTable(){ const table=$('#padTable'); if(!table) return; table.innerHTML=`<tr><th>Pad</th><th>MIDI Note</th><th>Name</th><th>Learn</th></tr>`+MapState.padNotes.map((m,i)=>{ const opts=DRUM_CHOICES.map(([name,n])=>`<option value="${n}" ${n===m?'selected':''}>${n} — ${name}</option>`).join(''); return `<tr><td>Pad ${i+1}</td><td><select data-pad-idx="${i}" class="padSel">${opts}</select></td><td>${m} (${midiName(m)})</td><td><button class="learnPad" data-idx="${i}">●</button></td></tr>` }).join(''); table.querySelectorAll('.padSel').forEach(sel=>{ sel.onchange=()=>{ const idx=+sel.dataset.padIdx; MapState.padNotes[idx]=parseInt(sel.value,10); saveAll(); updateInlineBadges(); }; }); table.querySelectorAll('.learnPad').forEach(btn=>{ btn.onclick=()=>{ startPadLearn(parseInt(btn.dataset.idx,10), btn); }; }); }
...
function bindConfig(){
  $('#exportBtn').onclick=()=>{ const obj={ ccMap:MapState.ccMap, ccMode:MapState.ccMode, padNotes:MapState.padNotes, padMode:state.padMode, drumKit:state.drumKit }; const txt=JSON.stringify(obj,null,2); navigator.clipboard?.writeText(txt); const pb=$('#pastebox'); pb.value = (pb.value ? pb.value + '' : '') + txt; };
...
buildKeyboard(state); buildPads(); injectInlineMap(); bindKeyMouse(); bindTyping(); bindMIDI(); bindUI(); loadAll(); updatePadModeUI(); renderCCTable(); renderPadTable(); bindConfig(); updateInlineBadges();

docReady();
function docReady(){ say('Ready. Start Audio → Test Tone → Connect MIDI.','ok'); diag(); ensurePresets(); Eng.setDrumKit(state.drumKit); window.addEventListener('visibilitychange',()=>diag()); }