...
export class Recorder{
  ...
  start(mime='audio/webm;codecs=opus'){
  if(!this.dest) this.arm('master');
  this.chunks=[];
  this.rec=new MediaRecorder(this.dest.stream,{mimeType:mime});
  this.rec.ondataavailable=e=>{ if(e.data && e.data.size) this.chunks.push(e.data) };
  this.rec.onstop=()=>{
    const blob=new Blob(this.chunks,{type:mime});
    const url=URL.createObjectURL(blob);
    const a=document.createElement('a'); a.href=url; a.download=`take-${this.which}.webm`; a.click();
  };
  this.rec.start(250); // <-- ensure periodic data delivery
}
