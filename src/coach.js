// Coach: metronome, arpeggiator, scale highlight, velocity curve, chord hints
// v2: help text, screen-safe position, toggle button, bug fixes
...
export class Coach{
  constructor({noteOnRaw, noteOffRaw, getKBEl}){
    ...
    this.chordHints=false;
  }

  mount(){ this.injectCSS(); this.buildPanel(); this.updateScale(); this.makeToggle(); }

  injectCSS(){
    const css=`#coach{position:fixed; right:16px; bottom:16px; width:min(360px,95vw); background:#0d0f11ee; backdrop-filter:blur(6px); border:1px solid #222; border-radius:16px; box-shadow:0 10px 30px rgba(0,0,0,.45); font-family:ui-sans-serif,system-ui; color:#eaeaea; z-index:9999}
...
#coach .help{display:none; font-size:12px; color:#bfc8d2; grid-column:1 / -1; background:#0f1317; border:1px solid #1d2126; border-radius:10px; padding:8px}
#kb .scale-ok{box-shadow: inset 0 0 0 2px #2ea043aa}
#kb .scale-no{opacity:.55}
#coachToggle{position:fixed; right:16px; bottom:16px; z-index:9998; background:#0b5bd3; color:#fff; border:none; border-radius:999px; padding:10px 14px; box-shadow:0 6px 16px rgba(0,0,0,.35);}
`;
    const style=document.createElement('style'); style.textContent=css; document.head.appendChild(style);
  }

  makeToggle(){
    let t=document.getElementById('coachToggle'); if(!t){ t=document.createElement('button'); t.id='coachToggle'; t.textContent='Coach'; document.body.appendChild(t); }
    t.onclick=()=>{ const p=document.getElementById('coach'); if(!p){ this.buildPanel(); return } p.style.display = (p.style.display==='none')?'block':'none'; };
  }

  buildPanel(){
    const wrap=document.createElement('div'); wrap.id='coach'; wrap.innerHTML=`
<header>Coach <span class="pill">learn & perform</span><button id="co-help" class="muted">Help</button></header>
<div class="sec" id="sec-metro">
  ...
  <div class="help" id="h-metro">Metronome: set BPM, choose beats-per-bar to accent beat 1. Tap button averages your last interval.</div>
</div>
<div class="sec" id="sec-arp">
  ...
  <div class="help" id="h-arp">Arpeggiator: hold 1–6 keys on your keyboard; it outputs notes in the chosen pattern at the selected rate. Gate is how long each step holds.</div>
</div>
<div class="sec" id="sec-scale">
  ...
  <div class="help" id="h-scale">Scale helper: highlights notes on the on-screen keyboard that fit the chosen key/scale. Great for improvising.</div>
</div>
<div class="sec" id="sec-chords">
  <label>Chord Hints</label><div class="row"><button id="co-chords">Off</button><span class="small muted">hold a note → highlights 3rd & 5th</span></div>
  <div class="help" id="h-chords">Chord hints: with Major scale, it suggests Major triads; with Minor, Minor triads. Try holding C — you’ll see E and G glow.</div>
</div>
<footer><span class="small">Tip: Start Audio, then Connect MIDI. Use the Coach button to hide/show.</span><button id="co-close" class="muted">Hide</button></footer>`;
    document.body.appendChild(wrap);

    // wire
    const $=s=>wrap.querySelector(s);
    $('#co-close').onclick=()=>{ wrap.style.display='none' };
    $('#co-help').onclick=()=>{ wrap.querySelectorAll('.help').forEach(el=>{ el.style.display = (el.style.display==='block')?'none':'block' }) };
    ...
    // Velocity curve
    const velEl=$('#co-vel'); if(velEl){ velEl.onchange=()=>{ this.velCurve=velEl.value } }
    ...
    // Chord hints
    const chBtn=$('#co-chords'); chBtn.onclick=()=>{ this.chordHints=!this.chordHints; chBtn.textContent=this.chordHints?'On':'Off' };
  }

  // -------- Metronome ---------
  startMetro(){ if(this.metroOn) return; this.metroOn=true; const ctx=this.ctx(); const tick=()=>{ const per=60/this.bpm; const acc=(this.metroCount%this.metroAccent)===0; const f=acc?1600:1000; const g=acc?0.22:0.16; this.beep(f,g,0.04); this.metroCount++; this.metroTimer=setTimeout(tick, per*1000); }; tick(); }
  ...
  // -------- Arp ---------
  noteOn(m, vel=110){ const v=this.shapeVel(vel); if(!this.arpOn){ this.noteOnRaw(m,v); this.applyHints(); return } if(!this.heldSet.has(m)){ this.heldSet.add(m); this.held=[...this.heldSet].sort((a,b)=>a-b) } if(!this.arpTimer) this.runArp(); this.applyHints(m) }
  noteOff(m){ if(!this.arpOn){ this.noteOffRaw(m); this.applyHints(); return } this.heldSet.delete(m); this.held=[...this.heldSet].sort((a,b)=>a-b); if(this.held.length===0) this.flushArp(); this.applyHints() }
  ...
  applyHints(root){ const kb=this.getKBEl?.(); if(!kb) return; [...kb.querySelectorAll('.hint')].forEach(el=>el.classList.remove('hint')); if(!this.chordHints || root==null) return; const isMinor=(this.scaleType.includes('minor')||this.scaleType==='dorian'); const third=root+(isMinor?3:4); const fifth=root+7; for(const m of [third,fifth]){ const el=kb.querySelector(`[data-midi="${m}"]`); if(el) el.classList.add('hint') } }
